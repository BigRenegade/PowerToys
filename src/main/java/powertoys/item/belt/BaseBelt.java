...
public class BaseBelt extends PRItem implements IBauble
{
	@GameRegistry.ObjectHolder(Reference.BASEBELT)
	...
	public void onWornTick(ItemStack stack, EntityLivingBase player) {
		...
		if (stack.getItemDamage()==0)
			if  (player.ticksExisted%120==0) {
				player.addPotionEffect(new PotionEffect(MobEffects.FIRE_RESISTANCE,20*60,0,true,false));
				player.addPotionEffect(new PotionEffect(MobEffects.WATER_BREATHING,20*60,0,true,false));
			}
                ...
                if (blockpos$mutableblockpos1.distanceSqToCenter(player.posX, player.posY, player.posZ) <= (double)(f * f))
                {
                    ...
                    if (iblockstate.getMaterial() == Material.AIR)
                    {
                        ...
                        if (iblockstate1.getMaterial() == Material.WATER && (iblockstate1.getBlock() == net.minecraft.init.Blocks.WATER || iblockstate1.getBlock() == net.minecraft.init.Blocks.FLOWING_WATER) && ((Integer)iblockstate1.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.mayPlace(Blocks.FROSTED_ICE, blockpos$mutableblockpos1, false, EnumFacing.DOWN, (Entity)null))
                        {
                            ...
                            worldIn.scheduleUpdate(blockpos$mutableblockpos1.toImmutable(), Blocks.FROSTED_ICE, MathHelper.getInt(player.getRNG(), 60, 120));
                        }

                        if (iblockstate1.getMaterial() == Material.LAVA && (iblockstate1.getBlock() == net.minecraft.init.Blocks.LAVA || iblockstate1.getBlock() == net.minecraft.init.Blocks.FLOWING_LAVA) && ((Integer)iblockstate1.getValue(BlockLiquid.LEVEL)).intValue() == 0 && worldIn.mayPlace(Blocks.COBBLESTONE, blockpos$mutableblockpos1, false, EnumFacing.DOWN, (Entity)null))
                        {
                        	worldIn.setBlockState(blockpos$mutableblockpos1, Blocks.COBBLESTONE.getDefaultState());
                            worldIn.scheduleUpdate(blockpos$mutableblockpos1.toImmutable(), Blocks.COBBLESTONE, MathHelper.getInt(player.getRNG(), 60, 120));
                        }
                    }
                }
            }
