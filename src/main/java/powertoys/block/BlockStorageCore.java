...
public class BlockStorageCore extends PRBlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem,
		...
		if (!worldIn.isRemote) {
			...
			if (tileEntity.hasCraftBox) {
				playerIn.openGui(PowerToys.instance, GuiHandler.CRAFTING, worldIn, pos.getX(), pos.getY(), pos.getZ());
			} else {
				playerIn.openGui(PowerToys.instance, GuiHandler.STORAGE, worldIn, pos.getX(), pos.getY(), pos.getZ());
			}
