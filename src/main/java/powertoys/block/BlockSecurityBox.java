...
public class BlockSecurityBox extends PRBlockContainer {
	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem,
		...
		if (tile.isPlayerAllowed(playerIn)) { // allowed player actions:
			if (!heldItem.isEmpty() && heldItem.getItem() == PRItems.key) { // try to break the block
				if (!worldIn.isRemote)
					worldIn.destroyBlock(pos, true); // permitted block breaking
				return true;
			} else { // simply access the block GUI
				playerIn.openGui(PowerToys.instance, GuiHandler.SECURITY, worldIn, pos.getX(), pos.getY(), pos.getZ());
				return true;
			}
		}
		return false;
