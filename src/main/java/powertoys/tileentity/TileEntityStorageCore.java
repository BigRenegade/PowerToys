...
import powertoys.gui.server.InventoryExtractList;
import powertoys.init.PRBlocks;
import powertoys.network.MessageFilterUpdate;
import powertoys.tileentity.TileEntityExtractPort.EnumListMode;
import powertoys.util.BlockRef;
import powertoys.util.ItemGroup;
import powertoys.util.Log;
import powertoys.util.PRInventory;
import powertoys.util.PRStorageUtils;
import powertoys.util.ItemGroup.EnumSortMode;

/** The storage core tile entity */
public class TileEntityStorageCore extends TileEntityBase {

	public PRInventory inventory;
	...
	public TileEntityStorageCore() {
		inventory = new PRInventory(this);
	}
	...
	public void updateInventory() {
		updateTileEntity();
		PowerToys.nw.sendToDimension(new MessageFilterUpdate(this), world.provider.getDimension());
	}

	/** Updates the tile entity position in the world and marks it to be saved */
	public void updateTileEntity() {
		PRStorageUtils.notifyBlockUpdate(this);
		this.markDirty();
	}
	...
	private void getValidNeighbors(BlockRef br) {
		List<BlockRef> neighbors = PRStorageUtils.getNeighbors(br.pos.getX(), br.pos.getY(), br.pos.getZ(), world);
	...
	public boolean validateSystem() {
		...
		for (BlockRef ref : multiblock) {
			...
			if (count > 1) {
				...
				} else if (world.getBlockState(pos).getBlock() == PRBlocks.storage_core) {
					world.setBlockToAir(getPos());
					world.spawnEntity(new EntityItem(world, pos.getX(), pos.getY(), pos.getZ(), new ItemStack(PRBlocks.storage_core)));
				}
				return false;
			}
		}
